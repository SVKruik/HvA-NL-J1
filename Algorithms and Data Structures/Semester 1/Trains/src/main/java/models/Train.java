...
public class Train {
    ...
    public int getTotalMaxWeight() {
        if (this.isPassengerTrain()) return 0;
    ...
    public Wagon findWagonAtPosition(int position) {
        ...
        if (position == 0) return this.getFirstWagon();

        Wagon current = this.getFirstWagon();
        int currentPosition = 0;

        while (current != null) {
            if (currentPosition == position) return current;
    ...
    public Wagon findWagonById(int wagonId) {
        if (this.hasWagons() && this.getFirstWagon().getId() == wagonId) return this.getFirstWagon();
        Wagon current = this.getFirstWagon();

        while (current != null) {
            if (current.getId() == wagonId) return current;
            current = current.getNextWagon();
        }
    ...
    public boolean canAttach(Wagon wagon) {
        ...
        // Type
        if (this.isPassengerTrain() && wagon instanceof FreightWagon && this.hasWagons()) return false;
        if (this.isFreightTrain() && wagon instanceof PassengerWagon && this.hasWagons()) return false;

        // Capacity
        if (this.getNumberOfWagons() + incomingWagons > this.getEngine().getMaxWagons()) return false;

        // Already present
        Wagon current = this.getFirstWagon();
        while (current != null) {
            if (current == wagon) return false;
            current = current.getNextWagon();
        }
    ...
    public boolean insertAtFront(Wagon wagon) {
        ...
        if (this.getFirstWagon() == wagon) return true;
    ...
    public boolean insertAtPosition(int position, Wagon wagon) {
        ...
        if (position == 0 || !hasWagons()) {
            ...
            // Loop through the wagons
            Wagon current = this.getFirstWagon();
            int currentPosition = 0;
            while (current != null) {
                if (currentPosition == position) {
                    Wagon previousWagon = current.getPreviousWagon();

                    // Replace current wagon with new wagon
                    if (current.hasPreviousWagon()) {
                        current.detachFront();
                        wagon.detachFront();
                        previousWagon.attachTail(wagon);
                        wagon.getLastWagonAttached().attachTail(current);
                    } else this.insertAtFront(wagon);
                    return true;
                } else {
                    currentPosition++;
    ...
    public boolean moveOneWagon(int wagonId, Train toTrain) {
        ...
        if (wagon == null || this == toTrain) return false;

        Wagon nextWagon = wagon.getNextWagon();
        if (!toTrain.canAttach(wagon)) return false;
        wagon.removeFromSequence();

        if (this.getFirstWagon() == wagon) this.insertAtFront(nextWagon);
    ...
    public boolean splitAtPosition(int position, Train toTrain) {
        if (!this.hasWagons()) return false;
        Wagon splitWagon = this.findWagonAtPosition(position);
        if (!toTrain.canAttach(splitWagon)) return false;
        if (position == 0) this.setFirstWagon(null);

        if (toTrain.getNumberOfWagons() > 0) {
            toTrain.attachToRear(splitWagon);
        } else toTrain.insertAtFront(splitWagon);
     ...
     */
    public void reverse() {
        if (hasWagons()) this.insertAtFront(this.getFirstWagon().reverseSequence());
    }

    @Override
    public String toString() {
        return "Train with engine " + engine + " rides from " + origin + " to " + destination + " and has first wagon: " + getFirstWagon();
    }
}

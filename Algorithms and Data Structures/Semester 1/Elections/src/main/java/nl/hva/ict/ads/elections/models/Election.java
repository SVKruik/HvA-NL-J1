...
public class Election {
    ...
    public Collection<PollingStation> getPollingStationsByZipCodeRange(String firstZipCode, String lastZipCode) {
        // Does not work unfortunately. I tried :-(
        // return this.constituencies.stream()
        //    .flatMap(constituency -> constituency.getPollingStationsByZipCodeRange(firstZipCode, lastZipCode).stream())
        //    .collect(Collectors.toList());

        return this.constituencies.stream()
                .flatMap(pol -> pol.getPollingStations().stream())
                .filter(pollingStation -> pollingStation.getZipCode()
                        .compareTo(firstZipCode) >= 0 &&
    ...
    public PollingStation findMostRepresentativePollingStation() {
        ...
        return this.constituencies.stream()
                ...
                .min(Comparator.comparingDouble(ps -> this.euclidianVotesDistributionDeviation(ps.getVotesByParty(), overallVotes)))
                .orElse(null);
    }
    ...
    private double euclidianVotesDistributionDeviation(Map<Party, Integer> votesCounts1, Map<Party, Integer> votesCounts2) {
        ...
        int totalNumberOfVotes1 = Election.integersSum(votesCounts1.values());
        int totalNumberOfVotes2 = Election.integersSum(votesCounts2.values());
    ...
    /**
     * Create and log a summary of a specific party.
     *
    ...
    /**
     * Create and log a summary of the entire election.
     *
     * @return summary
     */
    public String prepareSummary() {
        ...
        summary.append("Total number of candidates in the election = ").append(new DecimalFormat("#,###").format(totalCandidates)).append("\n\n");
        ...
        // Election Results
        List<Map.Entry<Party, Double>> sortedResults = Election.sortedElectionResultsByPartyPercentage(totalParties, getVotesByParty());
        ...
        summary.append("Polling stations in Amsterdam Wibautstraat area with zip codes ").append(rangeA).append("-").append(rangeB).append(":\n").append(pollingStationsByZipCode).append("\n\n");

        // Election Results Wibautstraat
        int limit = 10;
        List<Map.Entry<Party, Double>> top10Results = Election.sortedElectionResultsByPartyPercentage(limit, this.getVotesByPartyAcrossPollingStations(pollingStationsByZipCode));
        summary.append("Top 10 election results by party percentage in Amsterdam Wibautstraat area with zip codes ").append(rangeA).append("-").append(rangeB).append(":\n").append(top10Results).append("\n\n");

        // Most Representative Polling Station
        PollingStation mostRepresentativeStation = this.findMostRepresentativePollingStation();
        limit = 20;
        List<Map.Entry<Party, Double>> representativeResults = Election.sortedElectionResultsByPartyPercentage(limit, mostRepresentativeStation.getVotesByParty());

...
/**
 * A Constituency (kieskring) is a regional district (of multiple cities and
 * villages).
 * Every Constituency has a unique Id
 * Parties have to register their candidates at every Constituency where they
 * want to be electable.
 * Within each Constituency each candidate is uniquely identified by a rank in
 * its party.
 * That is his/her position on the ballot list
 * A party may register different candidates in different constituencies
 * and also allocate a different rank to the same candidate across different
 * constituencies.
 * Within a Constituency you find a number of PollingStations where the voting
 * takes place.
 * Every polling station in the Constituency will use the same ballot list of
 * parties and candidates
 * (Different Constituencies may have different ballot lists)
 * Polling stations are organised by zip code and id such that
 * Navigable SubSets of PollingStations within a range of zipcodes can be
...
public class Constituency {
    ...
    // Reusable Comparator for sorting polling stations by zipCode and id
    public static Comparator<PollingStation> POLLING_STATION_COMPARATOR = Comparator
            .comparing(PollingStation::getZipCode)
            .thenComparing(PollingStation::getId);
    // All candidates that have been registered at this constituency organised by
    // party and rank
    ...
    // such that Navigable Subsets of polling stations within a range of zipcodes
    ...
    public Constituency(int id, String name) {
        ...
        this.pollingStations = new TreeSet<>(POLLING_STATION_COMPARATOR);
    }

    /**
     * Auxiliary method for parsing the data from the EML files
     * This method can be used as-is and does not require your investigation or
     * extension.
     */
    ...
    /**
     * registers a candidate for participation in the election for his/her party in
     * this constituency.
     * The given rank indicates the position on the ballot list.
     * If the rank is already taken by another Candidate, this registration shall
     * fail and return false.
     * If the given candidate has been registered already at another rank, this
     * registration shall also fail and return false
     * Used by XML import
     *
     * @param candidate
    ...
    public boolean register(int rank, Candidate candidate) {
        ...
        NavigableMap<Integer, Candidate> partyCandidates = this.rankedCandidatesByParty
                .computeIfAbsent(candidate.getParty(), key -> new TreeMap<>());
    ...
    /**
     * retrieves the collection of parties that have registered one or more
     * candidates at this constituency
     *
     * @return
     */
    ...
    /**
     * retrieve a list of all registered candidates for a given party in order of
     * their rank
     *
     * @param party
     * @return
    ...
    /**
     * Retrieve a map of all candidates by rank.
     *
     * @param party
     * @return
     */
    ...
    /**
     * Retrieve the sub set of polling stations that are located within the area of
     * the specified zip codes
    ...
    public NavigableSet<PollingStation> getPollingStationsByZipCodeRange(String firstZipCode, String lastZipCode) {
        ...
        if (zipCode.isInvalidZipCode(firstZipCode) || zipCode.isInvalidZipCode(lastZipCode))
            return null;
    ...
    public boolean equals(Object o) {
        if (this == o)
            return true;
        if (!(o instanceof Constituency other))
            return false;

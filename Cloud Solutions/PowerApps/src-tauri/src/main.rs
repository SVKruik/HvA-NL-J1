...
#[tauri::command]
async fn cloud_submit(message: &str, source: &str, project: &str) -> Result<String, String> {
    // Prepare Payload
    dotenv().ok();
    let client = reqwest::Client::new();
    let url = env::var("CLOUD_SOLUTIONS_POWER_APPS_TRIGGER_URL").unwrap();
    let payload = json!({ "message": message, "source": source, "project": project});
    let payload_ser = serde_json::to_string(&payload)
        .map_err(|e| format!("Failed to serialize JSON payload: {:?}", e))?;

...
export default defineEventHandler(async (event): Promise<BuddyStatus> => {
    try {
        ...
        if (status === "accept") {
            const response: boolean = await new Buddy(null, hopperId, buddyId, connection).accept();
            if (!response) throw new Error("Failed to accept the Buddy request. Please try again later", { cause: { statusCode: 1500 } });
            await sendPeer({
                hopper_id: buddyId,
                type: NotificationTypes.buddyAccepted,
                data: {
                    buddy_id: hopperId,
            ...
            const response: boolean = await new Buddy(null, hopperId, buddyId, connection).decline();
            if (!response) throw new Error("Failed to decline the Buddy request. Please try again later.", { cause: { statusCode: 1500 } });
            await sendPeer({
                hopper_id: buddyId,
                type: NotificationTypes.buddyDeclined,
                data: {
                    buddy_id: hopperId,
                    username: hopper.username,
                    avatar: hopper.avatar,
